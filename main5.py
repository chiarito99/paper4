...
class FollowerUAV:
    ...
    def control_signal(self, ref,rbt_pos):
...
if __name__ == "__main__":
    ...
    x_end = 80
    y_end = 80
    K = [(30.3, 93.5), (40.3, 31.9), (65.3, 45), (110.1, 24.7), (151.5, 37.3), (117.5, 48.9), (156.1, 70.1)]
    ...
    path = get_path(K,resolution,-1)
    path.insert(0,[x_start,y_start])
    # print(path)
    map = Env1([x_start,y_start],[x_end,y_end],resolution,path)
    pt = map.point
    print(pt)
    # print(pt)
    ...
    follower1 = FollowerUAV(pos=[x_start,y_start],leader=leader, delta=[-offsetx,-offsety],wp = pt)
    follower2 = FollowerUAV(pos=[x_start,y_start],leader=leader, delta=[-offsetx, offsety],wp = pt)
    follower3 = FollowerUAV(pos=[x_start,y_start],leader=leader, delta=[-2*offsetx, -2*offsety],wp = pt)
    follower4 = FollowerUAV(pos=[x_start,y_start],leader=leader, delta=[-2*offsetx, 2*offsety],wp = pt)
    x_traj, y_traj = [], []
    for i in range(1,len(map.traj[0])):
        ...
        # UAV processing
        lvel = leader.control_signal(ref,flag,err1,err2,err3,err4)
        f1vel = follower1.control_signal(ref,rbt_pos)
        f2vel = follower2.control_signal(ref,rbt_pos)
        f3vel = follower3.control_signal(ref,rbt_pos)
        f4vel = follower4.control_signal(ref,rbt_pos)
        # UAV update
    ...
    plot.plot_animation(leader.path,follower1.path,follower2.path,follower3.path,follower4.path,ox, oy,x_start,y_start,x_end,y_end,length,width)
    plt.show()
    ...
    ax.plot(follower1.path[:,0], follower1.path[:,1], '--g', label='Follower 1')
    ax.plot(follower2.path[:,0], follower2.path[:,1], '--g', label='Follower 2')
    ax.plot(follower3.path[:,0], follower3.path[:,1], '--g', label='Follower 3')
    ax.plot(follower4.path[:,0], follower4.path[:,1], '--g', label='Follower 4')
